    ...
    >
      ...
      >
        <div slot="tags">
          ...
          <van-tag plain type="danger">{{m.wish}}想观看</van-tag>
        </div>
        <div slot="footer">
...
export default {
  ...
  methods: {
    onLoad() {
      ...
      this.$axios.get("http://localhost:3000/movie2").then(data => {
        this.movie2 = this.movie2.concat(data.data); //解构 然后在原有的数据上后面加新的数据
        window.console.log(data.data);

        // 加载状态结束
        this.loading = false;
        // 数据全部加载完成
        if (this.movie2.length >= 30) {
          this.finished = true;
        }
